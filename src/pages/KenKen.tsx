    ...
    <Layout userId={userId || ''} showBack backTo={`/${userId}/brain`} maxWidth="max-w-3xl">
      <div className="py-4">
        ...
        <div className="rounded-3xl bg-white dark:bg-gray-900 p-4 shadow-sm mb-4">
          ...
          <div className="grid grid-cols-4 border-2 border-gray-900 dark:border-gray-100 mx-auto max-w-[360px] aspect-square">
            {game.grid.map((row, r) => row.map((value, c) => {
              ...
              return <button key={`${r}-${c}`} onClick={() => game.setSelected({ row: r, col: c })} className={`relative bg-white dark:bg-gray-950 flex items-center justify-center text-2xl font-black text-gray-900 dark:text-gray-100 ${selected ? 'ring-4 ring-orange-400 ring-inset bg-orange-50 dark:bg-orange-900/20' : ''}`} style={{ borderTop: topSame ? '1px solid #d1d5db' : '3px solid #111827', borderLeft: leftSame ? '1px solid #d1d5db' : '3px solid #111827', borderRight: rightSame ? '1px solid #d1d5db' : '3px solid #111827', borderBottom: bottomSame ? '1px solid #d1d5db' : '3px solid #111827' }}>{first && <span className="absolute top-1 left-1 text-[11px] font-bold text-orange-700 dark:text-orange-300">{cage ? cageLabel(cage) : ''}</span>}{value ?? ''}</button>;
            }))}
          </div>
