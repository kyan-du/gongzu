...
import { CheckCircle, Clock, BookX, ChevronLeft, ChevronRight, BookOpen, Languages, Brain, Grid3X3, Ruler, Code, BookMarked, Calculator, Puzzle, ClipboardList } from 'lucide-react';
                    ...
                    <button key={quiz.id}
                      ...
                      <div className="flex items-center gap-3">
                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${
                          ...
                          quiz.tag.includes('应用题') ? 'bg-rose-50 dark:bg-rose-900/30' :
                          quiz.tag.includes('口算') ? 'bg-sky-50 dark:bg-sky-900/30' :
                          quiz.tag.includes('规律') ? 'bg-purple-50 dark:bg-purple-900/30' :
                          ...
                          {isMemGame ? <span className="text-xl">{gameType === 'matryoshka' ? '🎭' : '🔢'}</span> :
                           ...
                           quiz.tag.includes('应用题') ? <ClipboardList className="w-5 h-5 text-rose-500 dark:text-rose-400" /> :
                           quiz.tag.includes('口算') ? <Calculator className="w-5 h-5 text-sky-500 dark:text-sky-400" /> :
                           quiz.tag.includes('规律') ? <Puzzle className="w-5 h-5 text-purple-500 dark:text-purple-400" /> :
